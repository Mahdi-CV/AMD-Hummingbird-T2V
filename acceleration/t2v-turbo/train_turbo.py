...
def parse_args():
    ...
    parser.add_argument(
        "--dataset_dir",
        type=str,
        default="/group/ossdphi_algo_scratch_08/jieguo12/datasets/webvid/train",
        help="webvid data path",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
...
def main(args):
    ...
    os.environ['WDS_CACHE']=args.dataset_dir
    ...
    teacher_config = OmegaConf.load(args.teacher_model_cfg)
    teacher_model_config = teacher_config.pop("model", OmegaConf.create())
    teacher_t2v = instantiate_from_config(teacher_model_config)
    teacher_t2v = load_model_checkpoint(
        teacher_t2v,
        args.teacher_model_path,
    )
    teacher_unet = teacher_t2v.model.diffusion_model
    ...
    unet_lora_params, unet_negation = lora_manager.add_lora_to_model(
        ...
        lora_path=args.unet_checkpoint, 
        r=args.lora_rank,
    )
    ...
    config.data.params.train.params.video_length = args.n_frames
    config.data.params.train.params.meta_path = args.train_shards_path_or_url
    ...
    num_train_examples = min(args.max_train_samples, len(data.datasets['train']))
    global_batch_size = config.data.params.batch_size * accelerator.num_processes
    # num_worker_batches = math.ceil(
    #     num_train_examples / (global_batch_size * num_workers)
    # )

    num_batches = num_train_examples / global_batch_size
    ...
    uncond_prompt_embeds = text_encoder.encode([""] * config.data.params.batch_size)
    ...
    logger.info(f"  Instantaneous batch size per device = {config.data.params.batch_size}")
    logger.info(
    ...
    if args.resume_from_checkpoint and not args.unet_checkpoint:
        path=None
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                text = batch["caption"]
                # Convert video from (b, c, t, h, w) to (b, t, c, h, w)
                video = video.permute(0, 2, 1, 3, 4)
                video = video.to(accelerator.device, non_blocking=True)
                pixel_values = video.to(dtype=weight_dtype)
                b, t = pixel_values.shape[:2]
                pixel_values_flatten = pixel_values.reshape(b * t, *pixel_values.shape[2:])
                ...
                if (
                    ...
                    assert config.data.params.batch_size == 1
                    idx = torch.randint(0, t, (args.reward_batch_size,))

                    selected_latents = (
                        model_pred[:, :, idx].to(vae.dtype) / vae_scale_factor
                    )
                    num_images = config.data.params.batch_size * args.reward_batch_size
                ...
                if (
                    ...
                    num_images = config.data.params.batch_size * args.video_rm_batch_size
                    selected_latents = selected_latents.permute(0, 2, 1, 3, 4)
                    selected_latents = selected_latents.reshape(
                        num_images, *selected_latents.shape[2:]
                    )
