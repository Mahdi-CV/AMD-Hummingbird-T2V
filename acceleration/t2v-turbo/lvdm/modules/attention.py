...
FLASH_IS_AVAILBLE = False
XFORMERS_IS_AVAILBLE = False

try:
    from flash_attn import flash_attn_func
    assert not 'V100' in torch.cuda.get_device_name()
    print('flash attn available')
    FLASH_IS_AVAILBLE = True
except:
    FLASH_IS_AVAILBLE = False
    try:
        import xformers
...
class CrossAttention(nn.Module):
    ...
    def efficient_forward_flash_attention(self, x, context=None, mask=None):
        ...
        q, k, v = map(
            lambda t: t.unsqueeze(3)
            .reshape(b, t.shape[1], self.heads, self.dim_head).contiguous(),
            # .permute(0, 2, 1, 3)
            # .reshape(b * self.heads, t.shape[1], self.dim_head)
            # .contiguous(),
            (q, k, v),
        ...
        # Combine q, k, v for flash_attn
        # q, k, v = map(
        #     lambda t: t.reshape(b, t.shape[1], self.heads, self.dim_head),
        #     (q, k, v),
        # )
        # Optional attention bias (None in this case)
        # Compute attention using flash-attn
        # print(q.dtype, k.dtype, v.dtype)
        # exit(0)
        out = flash_attn_func(q,k,v, causal=False)  # causal=True for autoregressive models
        ...
        if k_ip is not None:
            ...
            # Optional attention bias (None in this case)
            # Compute attention using flash-attn
            out_ip = flash_attn_func(q, k_ip, v_ip, causal=False)  # causal=True for autoregressive models

            out_ip = (
                # out_ip.unsqueeze(0)
                # .reshape(b, self.heads, out.shape[1], self.dim_head)
                # .permute(0, 2, 1, 3)
                out_ip.reshape(b, out.shape[1], self.heads * self.dim_head)
            )
