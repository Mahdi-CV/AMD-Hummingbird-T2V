...
def load_model_checkpoint(model, ckpt, strict=True):
    def load_checkpoint(model, ckpt, full_strict):
        state_dict = torch.load(ckpt, map_location="cpu")
        if "state_dict" in state_dict.keys():
            state_dict = state_dict["state_dict"]
        elif 'module' in state_dict.keys():
            state_dict = {k[16:]: v for k, v in state_dict['module'].items() if not k[16:].startswith('teacher')}
        try:
            model.load_state_dict(state_dict, strict=full_strict)
        except:
            input_replace_dict = {
                # 'model.diffusion_model.input_blocks.1.': 'model.diffusion_model.input_blocks.2.', 
                'model.diffusion_model.input_blocks.3.': 'model.diffusion_model.input_blocks.2.', 
                'model.diffusion_model.input_blocks.4.': 'model.diffusion_model.input_blocks.3.', 
                'model.diffusion_model.input_blocks.6.': 'model.diffusion_model.input_blocks.4.', 
                'model.diffusion_model.input_blocks.7.': 'model.diffusion_model.input_blocks.5.', 
                'model.diffusion_model.input_blocks.9.': 'model.diffusion_model.input_blocks.6.', 
                'model.diffusion_model.input_blocks.10': 'model.diffusion_model.input_blocks.7', 
            }
            output_replace_dict = {
                'model.diffusion_model.output_blocks.1.': 'model.diffusion_model.output_blocks.0.', 
                'model.diffusion_model.output_blocks.2.': 'model.diffusion_model.output_blocks.1.', 
                'model.diffusion_model.output_blocks.4.': 'model.diffusion_model.output_blocks.2.', 
                'model.diffusion_model.output_blocks.5.': 'model.diffusion_model.output_blocks.3.', 
                'model.diffusion_model.output_blocks.6.': 'model.diffusion_model.output_blocks.4.', 
                'model.diffusion_model.output_blocks.8.': 'model.diffusion_model.output_blocks.5.', 
                'model.diffusion_model.output_blocks.9.': 'model.diffusion_model.output_blocks.6.', 
                'model.diffusion_model.output_blocks.11': 'model.diffusion_model.output_blocks.7', 
            }
            keys = list(state_dict.keys())
            for name in keys:
                if name[:37] in input_replace_dict.keys():
                    state_dict[name.replace(name[:37], input_replace_dict[name[:37]])] = state_dict[name]
                if name[:38] in output_replace_dict.keys():
                    state_dict[name.replace(name[:38], output_replace_dict[name[:38]])] = state_dict[name]
            model.load_state_dict(state_dict, strict=full_strict)
        del state_dict
        gc.collect()
        return model
