...
class TemporalConvBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        return identity + x
def set_param_grad(model, need_grad):
...
class UNetModel(nn.Module):
    ...
    def __init__(self,
        ...
        if self.training_part != 'all':
            set_param_grad(self.time_embed, False)
        if fs_condition:
            ...
            # TODO: must finetuning?
            if self.training_part != 'all':
                set_param_grad(self.fps_embedding, False)
        ## Input Block
        ...
        if self.training_part != 'all':
            set_param_grad(self.input_blocks, False)
        if self.addition_attention:
            ...
            if self.training_part in ['spatial', 'none']:
                set_param_grad(self.init_attn, False)
        ...
        for level, mult in enumerate(channel_mult):
            for _ in range(num_res_blocks):
                res_block = ResBlock(ch, time_embed_dim, dropout,
                        out_channels=mult * model_channels, dims=dims, use_checkpoint=use_checkpoint,
                        use_scale_shift_norm=use_scale_shift_norm, tempspatial_aware=tempspatial_aware,
                        use_temporal_conv=temporal_conv
                    )
                if self.training_part != 'all':
                    set_param_grad(res_block, False)
                layers = [
                    res_block
                ]
                ...
                if ds in attention_resolutions:
                    ...
                    spatial_transformer_block = SpatialTransformer(ch, num_heads, dim_head,
                            depth=transformer_depth, context_dim=context_dim, use_linear=use_linear,
                            use_checkpoint=use_checkpoint, disable_self_attn=False,
                            video_length=temporal_length, #image_cross_attention=self.image_cross_attention,
                            #image_cross_attention_scale_learnable=self.image_cross_attention_scale_learnable,
                        )
                    if self.training_part in ['temporal', 'none']:
                        set_param_grad(spatial_transformer_block, False)
                    layers.append(
                        spatial_transformer_block
                    )
                    if self.temporal_attention:
                        temporal_transformer_block = TemporalTransformer(ch, num_heads, dim_head,
                                depth=transformer_depth, context_dim=context_dim, use_linear=use_linear,
                        ...
                        if self.training_part in ['spatial', 'none']:
                            set_param_grad(temporal_transformer_block, False)
                        layers.append(
            ...
            if level != len(channel_mult) - 1:
                out_ch = ch
                added_block = TimestepEmbedSequential(
                ...
                if self.training_part != 'all':
                    set_param_grad(added_block, False)
        ...
        if num_head_channels == -1:
            ...
            num_heads = ch // num_head_channels
            dim_head = num_head_channels
        res_block, spatial_block = ResBlock(ch, time_embed_dim, dropout,
        ...
        if self.training_part != 'all':
            set_param_grad(res_block, False)
        if self.training_part in ['temporal', 'none']:
            set_param_grad(spatial_block, False)
        layers = [
            res_block, spatial_block
        ]
        if self.temporal_attention:
            temporal_block = TemporalTransformer(ch, num_heads, dim_head,
                    depth=transformer_depth, context_dim=context_dim, use_linear=use_linear,
                    use_checkpoint=use_checkpoint, only_self_att=temporal_self_att_only,
                    causal_attention=use_causal_attention, relative_position=use_relative_position,
                    temporal_length=temporal_length
            ...
            if self.training_part in ['spatial', 'none']:
                set_param_grad(temporal_block, False)
            layers.append(
                temporal_block
            )
        res_block =  ResBlock(ch, time_embed_dim, dropout,
        ...
        for level, mult in list(enumerate(channel_mult))[::-1]:
            for i in range(num_res_blocks + 1):
                ...
                res_block = ResBlock(ch + ich, time_embed_dim, dropout,
                        out_channels=mult * model_channels, dims=dims, use_checkpoint=use_checkpoint,
                        use_scale_shift_norm=use_scale_shift_norm, tempspatial_aware=tempspatial_aware,
                        use_temporal_conv=temporal_conv
                    )
                ...
                if ds in attention_resolutions:
                    if num_head_channels == -1:
                        ...
                        num_heads = ch // num_head_channels
                        dim_head = num_head_channels
                    spatial_block = SpatialTransformer(ch, num_heads, dim_head,
                            depth=transformer_depth, context_dim=context_dim, use_linear=use_linear,
                            use_checkpoint=use_checkpoint, disable_self_attn=False, video_length=temporal_length,
                            # image_cross_attention=self.image_cross_attention,image_cross_attention_scale_learnable=self.image_cross_attention_scale_learnable
                        )
