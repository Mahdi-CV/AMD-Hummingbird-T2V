...
class LatentDiffusion(DDPM):
    ...
    def apply_teacher_model(self, x_noisy, t, cond, **kwargs):
        if isinstance(cond, dict):
            # hybrid case, cond is exptected to be a dict
            pass
        else:
        ...
        x_recon = self.teacher_model(x_noisy, t, **cond, **kwargs)
    ...
    def p_losses(self, x_start, cond, t, noise=None, **kwargs):
        ...
        if self.k != 0:
            prev_t = (t - self.k).clip(0)
            prev_x_noisy = self.q_sample(x_start=x_start, t=prev_t, noise=noise)

            with torch.no_grad():
                target = self.apply_model(prev_x_noisy, prev_t, cond, **kwargs)

            target = self.predict_start_from_noise(prev_x_noisy, t=prev_t, noise=target)
            model_output = self.predict_start_from_noise(x_noisy, t=t, noise=model_output)

        else:
            teacher_t = t
            teacher_x_noisy = x_noisy

            with torch.no_grad():
                if self.use_cfg:
                    target = self.apply_teacher_model(teacher_x_noisy, teacher_t, cond, **kwargs)
                else:
                    cond_target = self.apply_teacher_model(teacher_x_noisy, teacher_t, cond, **kwargs)
                    uncond = {key:cond[key] for key in cond.keys() if key!='c_crossattn'}
                    uncond['c_crossattn'] = [self.uncond_emb.repeat(x_start.shape[0], 1, 1)]
                    uncond_target = self.apply_teacher_model(teacher_x_noisy, teacher_t, uncond, **kwargs)
                    target = uncond_target + self.distil_guidance_scale * (cond_target - uncond_target)

        loss_distil = self.get_loss(model_output, target)
        loss_dict.update({f'{prefix}/loss_distil': loss_distil})

        return loss_distil, loss_dict  
    ...
    def configure_schedulers(self, optimizer):
        ...
        elif 'steplr' in scheduler_name.lower():
            # scheduler = instantiate_from_config(self.scheduler_config)
...
class LatentVisualDiffusion(LatentDiffusion):
    def __init__(self, *args, **kwargs):
        ...
        assert self.k >= 0, 'k must be non negtive'
        self.distil_guidance_scale = kwargs.pop('distil_guidance_scale', 12.)
        self.use_cfg = kwargs.pop('use_cfg', True)
        super().__init__(*args, **kwargs)
        self.training_part = self.model.diffusion_model.training_part
        assert self.training_part in ['temporal','spatial','t+p','all','none'], self.training_part
        self.uncond_emb = None
    ...
    def get_batch_input(self, batch, random_uncond, return_first_stage_outputs=False, return_original_cond=False, return_fs=False, return_original_input=False, **kwargs):
        ## x: b c t h w
        x = super().get_input(batch, self.first_stage_key)
        ## encode video frames x to z via a 2D encoder        
        z = self.encode_first_stage(x)
        ...
        with torch.no_grad():
            ...
            self.uncond_emb = self.get_learned_conditioning([""]) if self.uncond_emb is None else self.uncond_emb
            
        cond = {}

        # whether use cfg
        if random_uncond and self.use_cfg:
            random_num = torch.rand(x.size(0), device=x.device)
            prompt_mask = rearrange(random_num < 2 * self.uncond_prob, "n -> n 1 1")

            cond_emb = torch.where(prompt_mask, self.uncond_emb, cond_emb.detach())
    ...
    def log_images(self, batch, sample=True, ddim_steps=50, ddim_eta=1., plot_denoise_rows=False, \
        ...
        z, c, xrec, xc, fs = self.get_batch_input(batch, random_uncond=False,
                                                return_first_stage_outputs=True,
                                                return_original_cond=True,
                                                return_fs=True)
