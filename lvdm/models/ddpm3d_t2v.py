...
class LatentVisualDiffusion(LatentDiffusion):
    ...
    def get_batch_input(self, batch, random_uncond, return_first_stage_outputs=False, return_original_cond=False, return_fs=False, return_original_input=False, **kwargs):
        ...
        cond["c_crossattn"] = [prompt_imb] ## concat in the seq_len dim
    ...
    def configure_optimizers(self):
        ...
        elif self.training_part == 'none':
            self.model.eval()
            self.model.train = False
            for param in self.model.parameters():
                param.requires_grad = False
            mainlogger.info(f"@NOT Training any parameters of unet.")
        else:
            assert self.training_part in ['temporal','spatial','t+p'], self.training_part
            params = [p for p in self.model.parameters() if p.requires_grad == True]
            mainlogger.info(f"@Training [{len(params)}] partial Paramters for {self.training_part}.")
