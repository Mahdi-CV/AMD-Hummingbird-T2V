...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument("--seed", "-s", type=int, default=20240723, help="seed for seed_everything")
...
if __name__ == "__main__":
    ...
    if config.model.params.k == 0:

        teacher_model = instantiate_from_config(config.teacher_model)
        teacher_model = load_checkpoints(teacher_model, config.teacher_model)

        model.teacher_model = teacher_model.model
        del(teacher_model)
