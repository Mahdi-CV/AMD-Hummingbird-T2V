...
def load_checkpoints(model, model_cfg, distil=False):
    if check_config_attribute(model_cfg, "pretrained_checkpoint"):
        ...
        if distil:
            ...
            keys = list(pl_sd['state_dict'].keys())
            for name in keys:
                if name[:37] in input_replace_dict.keys():
                    pl_sd['state_dict'][name.replace(name[:37], input_replace_dict[name[:37]])] = pl_sd['state_dict'][name]
                if name[:38] in output_replace_dict.keys():
                    pl_sd['state_dict'][name.replace(name[:38], output_replace_dict[name[:38]])] = pl_sd['state_dict'][name]
