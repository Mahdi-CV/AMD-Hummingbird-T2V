...
class Image2Video():
    def __init__(self,result_dir='./tmp/',gpu_num=1,resolution='256_256',temp_length=16) -> None:
        ...
        ckpt_path = '/mnt/share/amdsharks/mengmege/models/dynamicrafter'
        ckpt_path = os.path.join(ckpt_path, resolution.split('_')[-1], 'model.ckpt')
        config_file='configs/inference_'+resolution.split('_')[1]+'_v1.0.yaml'
        print(ckpt_path, config_file)
        assert os.path.isfile(ckpt_path) and os.path.isfile(config_file), ckpt_path + '----' + config_file
        config = OmegaConf.load(config_file)
        model_config = config.pop("model", OmegaConf.create())
        model_config['params']['unet_config']['params']['temporal_length'] = temp_length
